...
\begin{align}
\frac{du}{dx} &= 2\pi \\[4pt]
f_{X,1}(x) &= f_U(u)\left|\frac{du}{dx}\right| \\[4pt]
            &= \frac{1}{2\pi}\cdot 2\pi \\[4pt]
            &= 1
\end{align}
$$
...
\begin{align}
\frac{du}{dx} &= -2\pi \\[4pt]
\left|\frac{du}{dx}\right| &= 2\pi \\[4pt]
f_{X,2}(x) &= f_U(u)\left|\frac{du}{dx}\right| \\[4pt]
            &= \frac{1}{2\pi}\cdot 2\pi \\[4pt]
            &= 1
\end{align}
$$
...
$$
\begin{align}
f_X(x) &= f_{X,1}(x) + f_{X,2}(x) \\[4pt]
       &= 2, \quad 0 \leq x \leq \tfrac{1}{2}
\end{align}
$$
...
よって確率変数 $X$ は  
$$
X \sim \mathrm{Uniform}\!\left(0, \tfrac{1}{2}\right)
$$
に従う.
...
\begin{align}
Y &= 1 - X  \\[4pt]
\Rightarrow \mathrm{Cov}[X,Y] &= \mathrm{Cov}[X, 1-X].
\end{align}
$$
...
\begin{align}
\mathrm{Cov}[X,Y] 
   &= \mathrm{Cov}[X,1] - \mathrm{Cov}[X,X] \\[4pt]
   &= 0 - \mathrm{V}[X] \\[4pt]
   &= -\mathrm{V}[X]
\end{align}
$$
...
\rho_{X,Y} 
   &= \frac{-\mathrm{V}[X]}
           {\sqrt{\mathrm{V}[X]\,\mathrm{V}[Y]}} \\[6pt]
   &= \frac{-\mathrm{V}[X]}{\mathrm{V}[X]} \\[6pt]
   &= -1
\end{align}
$$
